...
export default function Vacations() {
    ...
    useEffect(() => {
        setTimeout(() => {
            ...
            (async () => {
                const res = await fetch(`http://localhost:5000/vacations/follow/${currUserId}`, { // The vacations user follow
                    method: 'GET',
                    headers: { 'content-type': 'application/json' },
                    credentials: "include"
                })
            ...
            (async () => {
                const res = await fetch(`http://localhost:5000/vacations/unfollow/${currUserId}`, { // The vacations user unfollow
                    method: 'GET',
                    headers: { 'content-type': 'application/json' },
                    credentials: "include"
                })
            ...
            (async () => {
                const res = await fetch(`http://localhost:5000/vacations/unfollow`, { // The vacations no one follow
                    method: 'GET',
                    headers: { 'content-type': 'application/json' },
                    credentials: "include"
                })
                    ...
                    <>
...

                        {
                            localStorage.role === "user" ?
                                <>
                                    <h1 className='h1header'>Howdy {localStorage.email} 😉</h1>
